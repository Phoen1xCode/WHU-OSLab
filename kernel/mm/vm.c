...
pagetable_t kernel_pagetable; // 保存 内核页表 的根指针 顶级页表 Sv39的L2表
...
pagetable_t kvmmake(void) {
  ...
  // uart registers
  kvmmap(kpgtbl, UART0, UART0, PAGESIZE, PTE_R | PTE_W);

  // virtio mmio disk interface
  kvmmap(kpgtbl, VIRTIO0, VIRTIO0, PAGESIZE, PTE_R | PTE_W);
  ...
  kvmmap(kpgtbl, TRAMPOLINE, (uint64)trampoline, PAGESIZE, PTE_R | PTE_X);
...
void uvmfirst(pagetable_t pagetable, uchar *src, uint sz) {
  ...
  mappages(pagetable, 0, PAGESIZE, (uint64)mem, PTE_W | PTE_R | PTE_X | PTE_U);
  memmove(mem, src, sz);
}
...
uint64 uvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz) {
  ...
  if (PAGEROUNDUP(newsz) < PAGEROUNDUP(oldsz)) {
    int npages = (PAGEROUNDUP(oldsz) - PAGEROUNDUP(newsz)) / PAGESIZE;
    uvmunmap(pagetable, PAGEROUNDUP(newsz), npages, 1);
  }
...
void uvmfree(pagetable_t pagetable, uint64 sz) {
  if (sz > 0)
    uvmunmap(pagetable, 0, PAGEROUNDUP(sz) / PAGESIZE, 1);
  freewalk(pagetable);
}
...
// require process structure (struct proc).
uint64 vmfault(pagetable_t pagetable, uint64 va, uint64 max_sz) {
  uint64 mem;
